...
function oe_module_texting_add_menu_item(MenuEvent $event)
{
    ...
    $menuItem->label = xlt("Patient Text Messages");
...
function oe_module_logs_add_menu_item(MenuEvent $event)
{
    ...
    $menuItem->target = 'tex';
    $menuItem->menu_id = 'tex0';
    $menuItem->label = xlt("Patient Text Log");
...
function renderTelehealthButton(Event $event)
{
    ?>
    <script>
        let tnavbar = document.querySelector('#myNavbar');
        let tbuttonTitle = '<?php echo xlt('Direct Telehealth') ?>';
        let tele = document.createElement("div");

        tele.id = "direct";
        tele.innerHTML = '<button class="btn btn-warning ml-2" id="directconnectelehealth">' + tbuttonTitle + '</button>';
        tnavbar.appendChild(tele);

        document
            .getElementById('directconnectelehealth')
...
$eventDispatcher->addListener(MenuEvent::MENU_UPDATE, 'oe_module_texting_add_menu_item', 10);
$eventDispatcher->addListener(MenuEvent::MENU_UPDATE, 'oe_module_bulktexting_add_menu_item', 10);
$eventDispatcher->addListener(MenuEvent::MENU_UPDATE, 'oe_module_settings_add_menu_item', 10);
$eventDispatcher->addListener(MenuEvent::MENU_UPDATE, 'oe_module_logs_add_menu_item', 10);
$eventDispatcher->addListener(RenderEvent::EVENT_RENDER_POST_PAGELOAD, 'renderTelehealthButton');
